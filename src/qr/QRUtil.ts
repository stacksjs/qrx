...
interface QrUtil {
  ...
  G18: number
  G15_MASK: number
  getBCHTypeInfo: (data: any) => any
  getBCHTypeNumber: (data: any) => any
  getBCHDigit: (data: any) => any
  getPatternPosition: (typeNumber: any) => number[]
  getMask: (maskPattern: any, i: number, j: number) => boolean
  getErrorCorrectPolynomial: (errorCorrectLength: number) => QRPolynomial
  getLengthInBits: (mode: number, type: any) => number
  getLostPoint: (qrCode: any) => number
}

export const QRUtil: QrUtil = {
