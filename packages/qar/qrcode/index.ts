 ...
 * - Modernized for TypeScript usage.
 ...
 */
function getAndroid(): number | boolean {
...
class QRPolynomial {
  ...
  constructor(num: number[], shift: number) {
    if (num.length === undefined) {
      throw new Error(`QRPolynomial constructor error: ${num}/${shift}`)
    }
  ...
  public multiply(e: QRPolynomial): QRPolynomial {
    const num = Array.from({ length: this.getLength() + e.getLength() - 1 }).fill(0)
...
export class QRCode {
  ...
  public makeCode(sText: string): void {
    ...
    if (typeof (this.oDrawing as CanvasDrawer).makeImage === 'function') {
      ...
      if (!this.android || this.android >= 3) {
        cDrawer.makeImage()
      }
    }
  }
